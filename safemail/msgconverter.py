...
class MsgConverter(object):
    ...
    def __init__(self,filename_or_stream = None):
        if filename_or_stream:
            try:
                self.file = self.load(filename_or_stream)
            except:
                raise AttributeError('unable to load provided file or stream!')

    def get(self):
        if self._ATTACHMENTS and self.file:
            return {
                'msg': self.file, 
                'attachments': self._ATTACHMENTS
            }
        elif self.file and not self._ATTACHMENTS:
            return {
                'msg': self.file,
                'attachments': None
            }
    ...
    def __add_transport_headers(self, props):
        ...
        for header, value in headers.items():
            self._MIME_MSG[header] = value


    def __add_common_headers(self, props):
        # Construct common headers from metadata.
        if 'MESSAGE_DELIVERY_TIME' in props:
            self._MIME_MSG['Date'] = formatdate(props['MESSAGE_DELIVERY_TIME'].timestamp())
        ...
        if 'SENDER_NAME' in props:
            ...
            if props['SENDER_NAME']:
                self._MIME_MSG['From'] = formataddr((props['SENDER_NAME'], ""))
            del props['SENDER_NAME']

        if 'DISPLAY_TO' in props:
            if props['DISPLAY_TO']:
                self._MIME_MSG['To'] = props['DISPLAY_TO']
            del props['DISPLAY_TO']

        if 'DISPLAY_CC' in props:
            if props['DISPLAY_CC']:
                self._MIME_MSG['CC'] = props['DISPLAY_CC']
            del props['DISPLAY_CC']

        if 'DISPLAY_BCC' in props:
            if props['DISPLAY_BCC']:
                self._MIME_MSG['BCC'] = props['DISPLAY_BCC']
            del props['DISPLAY_BCC']

        if 'SUBJECT' in props:
            if props['SUBJECT']:
                self._MIME_MSG['Subject'] = props['SUBJECT']
            del props['SUBJECT']
    ...
    def load_message_stream(self, entry, is_top_level, doc):
        ...
        if 'BODY' in props:
            ...
            if isinstance(body, str):
                ...
                self._MIME_MSG.set_content(body, maintype="text", subtype="plain", cte='8bit')
    ...
    def __process_attachment(self, entry, doc):
        # Load attachment stream.
        props = self.__parse_properties(entry['__properties_version1.0'], False, entry, doc)
        ...
        if isinstance(blob, str):
            ...
            self.__write_attachment(blob, filename)
    ...
    def __write_attachment(self,blob, filename):
        file_obj = BytesIO().write(blob)
        split_filename = filename.split('.',1)[0]
            ...
            while i < len(stream):
                ...
                # Variable Length Properties.
                    ...
                    try:
                        ...
                        print("stream missing: {}".format(streamname))
                        continue
                    ...
                    # unrecognized type
                    print("unhandled property type: {}".format(hex(property_type)))
                    continue
    ...
    @staticmethod
    def load_EMBEDDED_MESSAGE(self, entry, doc):
        return self.load_message_stream(entry, False, doc)
...
class EMBEDDED_MESSAGE(object):
    @staticmethod
    def load(entry, doc):
        return MsgConverter().load_message_stream(entry, False, doc)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-D', '--directory', action=readable_dir, help='Path to generate template package')
   # parser.add_argument('-N', '--name', help='The package name to generate a template for')
    parser.add_argument('-F', '--file', help='Force the creation of a template package')#action='store_true')
    ...
    if args.file:
        ...
        #if len(sys.argv) <= 1:
        msg = MsgConverter(args.file).get()
        with open(args.file + ".eml", "wb") as f:
